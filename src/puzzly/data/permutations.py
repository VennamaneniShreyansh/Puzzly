...
def generate_permutation_set(num_patches=9, num_permutations=50, seed=42, candidate_pool_size=2000):
    """Fixed set of permutations, chosen to be maximally different from each other
    (max-Hamming-distance greedy selection). Deterministic given the same seed."""
    ...
    for _ in range(num_permutations - 1):
        ...
        for cand in candidate_pool:
            min_dist = min(sum(a != b for a, b in zip(cand, existing)) for existing in chosen)
...
def save_permutation_set(perm_set, path="data/permutation_set.json"):
    with open(path, "w") as f:
        json.dump(perm_set, f)


def load_permutation_set(path="data/permutation_set.json"):
    with open(path) as f:
